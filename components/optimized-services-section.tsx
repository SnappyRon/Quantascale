...
export function OptimizedServicesSection() {
  ...
  return (
    <section id="services" className="py-24 bg-gray-950 relative overflow-hidden">
      ...
      <div
        className={`absolute top-0 left-1/3 w-96 h-96 bg-sky-600/5 rounded-full blur-3xl ${
      ...
      <div className="relative container mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {services.map((service, index) => (
            <OptimizedScrollReveal key={index} delay={prefersReducedMotion ? 0 : 200 + index * 100}>
              <Card
                className={`group bg-gray-900/50 border-gray-800/50 backdrop-blur-sm hover:bg-gray-900/70 transition-all duration-300 hover:border-gray-700/50 rounded-2xl cursor-pointer ${
                  !prefersReducedMotion && !isLowEndDevice ? "hover:scale-[1.02] transform" : ""
                }`}
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div
                      className={`p-4 bg-gradient-to-br ${service.gradient} text-white mb-6 rounded-2xl shadow-lg transition-all duration-300 ${
                        !prefersReducedMotion && !isLowEndDevice ? "group-hover:scale-110 group-hover:rotate-3" : ""
                      }`}
                    ...
                    <ArrowUpRight
                      className={`h-6 w-6 text-gray-500 group-hover:text-sky-400 transition-all duration-300 ${
                        !prefersReducedMotion ? "group-hover:translate-x-1 group-hover:-translate-y-1" : ""
                      }`}
                    />
                  </div>
                  <CardTitle className="text-2xl font-light text-white transition-colors">{service.title}</CardTitle>
                </CardHeader>
                ...
                <CardContent>
                  ...
                  <ul className="space-y-4">
                    {service.features.map((feature, featureIndex) => (
                      <li
                        key={featureIndex}
                        className={`flex items-center text-gray-300 group-hover:text-white transition-all duration-300 font-light ${
                          !prefersReducedMotion ? "group-hover:translate-x-2" : ""
                        }`}
                        style={{
                          transitionDelay: prefersReducedMotion ? "0ms" : `${featureIndex * 50}ms`,
                          willChange: "transform",
                        }}
                      >
                        <div
                          className={`w-1.5 h-1.5 bg-sky-400 rounded-full mr-4 transition-transform duration-300 ${
                            !prefersReducedMotion ? "group-hover:scale-125" : ""
                          }`}
                        />
                        {feature}
                      </li>
